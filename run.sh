...
python -u tool_name_accuracy.py
if [ $? -eq 0 ]; then
    echo "✅ 工具名正确率评估完成"
else
    echo "❌ 工具名正确率评估失败"
...
python -u schema_validation_accuracy.py
if [ $? -eq 0 ]; then
    echo "✅ Schema合法率评估完成"
else
    echo "❌ Schema合法率评估失败"
...
python -u query_param_accuracy.py
if [ $? -eq 0 ]; then
...
python -u obs_param_accuracy.py
if [ $? -eq 0 ]; then
...
python -u duplicate_call_rate.py
if [ $? -eq 0 ]; then
    echo "✅ 重复调用率评估完成"
else
    echo "❌ 重复调用率评估失败"
...
echo "=========================================="
python -u generate_metrics_csv.py
if [ $? -eq 0 ]; then
    echo "✅ CSV统计文件生成完成"
else
    echo "❌ CSV统计文件生成失败"
